...
def confirm_payment(text):
    ...
    if dm:
        d,mn,y=dm.groups()
        try:
            txn_date_str=f"{d}/{mn}/20{y}"
            dt=datetime.strptime(txn_date_str, '%d/%m/%Y')
            if dt.date() < (datetime.now()-timedelta(days=2)).date():
                return False, f"OLD DATE {txn_date_str}", txn_date_str, txn_id, amount
        except: pass
...
def start(m):
    ...
    bot.send_message(m.chat.id, "✅ $2 Auto Bot\n✅ Date + Txn Confirmed\n✅ Notify Danil ON\n👇 Choose $2 file:", reply_markup=markup)
...
def ph(m):
    ...
    markup=InlineKeyboardMarkup()
    markup.add(InlineKeyboardButton("✅ APPROVE & NOTIFY", callback_data=f"approve_{m.from_user.id}_{vtype}"))
    bot.send_photo(ADMIN_ID, m.photo[-1].file_id, caption=f"📸 Check\nApp:{vtype}\nUser:{m.from_user.id} @{m.from_user.username}\nDate must be {datetime.now().strftime('%d/%m/%Y')}", reply_markup=markup)
    bot.send_message(m.chat.id, "Screenshot sent to Danil for $2 check!")
...
def txt(m):
    ...
    if valid:
        ...
        if "ALREADY USED" in reason:
            ...
            bot.send_message(m.chat.id, f"❌ {reason}\nNeed USD 2.00 + Approval Code + TODAY date")
